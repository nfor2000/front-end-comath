...
const PreviewVideo = () => {
     ...
     useEffect(() => {
          ...
          const fetchData = async () => {
               try {
                 const contentRes = await axios.get(`http://ec2-16-171-60-144.eu-north-1.compute.amazonaws.com:4000/content/video/${id}`);
                 console.log(contentRes.data[0]);
                 setContent(contentRes.data[0]);
             
                 const teacherId = contentRes.data[0].teacher;
                 const teacherRes = await axios.get(`http://ec2-16-171-60-144.eu-north-1.compute.amazonaws.com:4000/teacher/profile/${teacherId}`);
